...
The bracket system is implemented in `core/synapse/context/context-tracker.js`.
...
The output formatter (`core/synapse/output/formatter.js`) enforces token budgets:
...
| File | Purpose |
|------|---------|
| `core/synapse/context/context-tracker.js` | Bracket calculation, token budgets, layer configs |
| `.synapse/context` | Bracket-specific context rules (L1) |
| `core/synapse/output/formatter.js` | Token budget enforcement + truncation |
