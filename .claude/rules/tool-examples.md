...
- **Pre-commit:** `wsl bash -c 'cd /mnt/c/.../project && ~/.local/bin/coderabbit --prompt-only -t uncommitted'`
- **Pre-PR:** `wsl bash -c 'cd /mnt/c/.../project && ~/.local/bin/coderabbit --prompt-only --base main'`
...
Full examples registry: `data/mcp-tool-examples.yaml`
Tool registry: `data/tool-registry.yaml`
